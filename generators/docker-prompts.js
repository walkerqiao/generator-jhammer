    ...
    const prompts = [
        {
            ...
            choices: [
                {
                    value: 'zuul',
                    name: 'JHammer gateway based on Netflix Zuul'
                },
                {
                    value: 'traefik',
...
function askForPath() {
    ...
    this.prompt(prompts).then(props => {
        ...
        for (let i = this.appsFolders.length - 1; i >= 0; i--) {
            if (this.appsFolders[i] === 'jhammer-registry' || this.appsFolders[i] === 'registry') {
                ...
                },
                {
                    ...
                    name:
                        this.deploymentApplicationType === 'monolith'
                            ? 'Yes, for logs and metrics with the JHammer Console (based on ELK)'
                            : 'Yes, for logs and metrics with the JHammer Console (based on ELK and Zipkin)'
                },
                {
                    value: 'prometheus',
                    name: 'Yes, for metrics only with Prometheus (only compatible with JHammer >= v3.12)'
                }
            ],
...
function askForConsoleOptions() {
    ...
    const prompts = [
        {
            ...
            name: 'consoleOptions',
            message:
                'You have selected the JHammer Console which is based on the ELK stack and additional technologies, which one do you want to use ?',
            choices: [
                {
                    value: 'curator',
    ...
    if (this.deploymentApplicationType === 'microservice') {
        prompts[0].choices.push({
            value: 'zipkin',
            name: 'Zipkin, for distributed tracing (only compatible with JHammer >= v4.2.0)'
        });
    }
    this.prompt(prompts).then(props => {
...
function askForServiceDiscovery() {
    ...
    if (serviceDiscoveryEnabledApps.every(app => app.serviceDiscoveryType === 'consul')) {
        ...
        this.log(chalk.green('JHammer registry detected as the service discovery and configuration provider used by your apps'));
        done();
    } else {
    ...
    const prompts = [
        {
            type: 'input',
            name: 'adminPassword',
            message: 'Enter the admin password used to secure the JHammer Registry',
            default: 'admin',
        ...
        if (file.isDirectory()) {
            if (
                ...
                try {
                    ...
                    if (
                        fileData['generator-jhammer'].baseName !== undefined &&
                        (deploymentApplicationType === undefined ||
                            deploymentApplicationType === fileData['generator-jhammer'].applicationType ||
                            (deploymentApplicationType === 'microservice' &&
                                fileData['generator-jhammer'].applicationType === 'gateway') ||
                            (deploymentApplicationType === 'microservice' && fileData['generator-jhammer'].applicationType === 'uaa'))
                    ) {
