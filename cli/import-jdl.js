...
const jhammerUtils = require('../generators/utils');
...
function importJDL() {
    ...
    const jdlImporter = new jhaCore.JDLImporter(this.jdlFiles, {
...
class JDLProcessor {
    ...
    getConfig() {
        if (jhaCore.FileUtils.doesFileExist('.yo-rc.json')) {
            logger.info('Found .yo-rc.json on path. This is an existing app');
            const configuration = jhammerUtils.getAllJhammerConfig(null, true);
            this.applicationType = configuration.applicationType;
            this.baseName = configuration.baseName;
            this.databaseType = configuration.databaseType || jhapsterUtils.getDBTypeFromDBValue(this.options.db);
